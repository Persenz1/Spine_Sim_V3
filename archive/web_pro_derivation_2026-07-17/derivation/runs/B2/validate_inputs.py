...
EXPECTED_UPLOADS = (
    ...
    "docs/extract/机理提取/06_soft_hard_array_scaling/06_soft_hard_array_scaling.zip",
    "docs/extract/机理提取/07_linearly_constrained_spines/07_linearly_constrained_spines.zip",
    "docs/extract/机理提取/10_soft_spiny_paw_load_sharing/10_soft_spiny_paw_load_sharing.zip",
)
...
def main() -> int:
    ...
    assert run["run_directory"] == "derivation/runs/B2"
    ...
    assert prerequisite["prior_module_run"] == "B1-r01"
    assert prerequisite["status"] == "accepted"
    assert prerequisite["current_module_context_version"] == "0.1.0"
    assert prerequisite["current_module_context_stage"] == "B1"
    ...
    if sum(path.endswith(".zip") for path in paths) != 3:
        raise ValueError("B2 最小文献包必须恰为 3 个 zip")
    ...
    prompt_path = ROOT / "derivation/prompts/B/B2_PROMPT.md"
    ...
    if "本轮网页端任务必须按下表顺序上传 12 个文件" not in prompt:
    ...
    for marker in (
        ...
        if marker not in current_text:
            raise ValueError(f"B 当前上下文缺少接受标记：{marker}")
    ...
    print("- 文献 06、07、10 压缩包结构、关键图数量和 CRC 通过")
