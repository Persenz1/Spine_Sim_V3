...
MANIFEST_PATH = ROOT / "derivation/runs/A3/INPUT_MANIFEST.yaml"
...
def main() -> int:
    ...
    assert run["id"] == "A3-r01"
    assert run["module"] == "A3"
    assert run["run_directory"] == "derivation/runs/A3"
    ...
    assert contract["expected_file_count"] == 12
    assert contract["verified_file_count"] == 12
    ...
    prompt_path = ROOT / "derivation/prompts/A/A3_PROMPT.md"
    ...
    if archive_content != prompt_path.read_bytes():
        raise ValueError("PROMPT.md 与正式 A3_PROMPT.md 不一致")
    ...
    for literature_path in paths[-4:]:
    ...
    print("- 文献 03、05、14、15 压缩包结构和 CRC 通过")
    return 0
