...
令 \(\mathbf P_\parallel=\mathbf a\mathbf a^{\mathsf T}\)、\(\mathbf P_\perp=\mathbf I-\mathbf P_\parallel\)、\(\mathbf S=[\mathbf a]_\times\)。针尖截面 wrench 为 \(\mathbf W_c=[\mathbf F_c;\mathbf M_c^{c_t}]\)。线性 Euler–Bernoulli 柔顺为
...
材料方向变换后，主线首次失效域由版本化的方向性/压力敏感 Mohr–Coulomb、拉伸截断、可选压缩帽和经标定的可选力矩容量组成：

\[
\Phi_{\rm init}
=\max(\Phi_{MC},\Phi_t,\Phi_c\ {\rm if\ enabled},
\Phi_M\ {\rm if\ calibrated})\le0.
\]

将无损容量集合写为 \(\mathcal C_{0,k}\)，定义射线利用率

\[
\boxed{
r_k^0(\boldsymbol\sigma)
=\inf\{\gamma>0:
\boldsymbol\sigma/\gamma\in\mathcal C_{0,k}\}.
}
\]
...
弹簧到硬限位后，其 \(\mathbf a\mathbf a^{\mathsf T}/k_s\) 项消失，切向斜率相应增大。若梁、弹簧和局部接触柔顺全部关闭，则 \(C_{xx}=0\)：理想固定支持不应出现任意平滑有限斜率；响应属于刚性约束/几何事件或集合值分支。任何有限斜率都必须来自显式启用且可追溯的物理柔顺。
...
摩擦/损伤分支无负耗散方向。该条件只声明局部增量稳定，不声明全局 Lyapunov 稳定。
...
本文件完成的是模型、方程、状态、事件、算法和合同的候选规范。尚未完成求解器代码、材料/针体参数标定、CAD 补齐、容差收敛、随机样本研究或实验验证。解析“通过”仅指公式/结构闭合，不得表述为已运行的软件或已验证实验。
...
| 状态、方程、符号、单位、功、事件和事务全局唯一 | 本文件已完成候选冻结 | 代码数据模型审查 |
...
因此当前状态仍为 `candidate`：规范层完成，不等于软件、参数或实验层完成。
...
| 原登记 ID | 本候选中的状态 | 当前安全处理 | 关闭条件/允许关闭层 |
|---|---|---|---|
...
| `A_model` | `A_INTEGRATED_MODEL 1.0.0-candidate` |
...
| 输入字段 | 必须内容与语义 |
|---|---|
| `contract_id`, `contract_version`, `call_mode` | 必须分别为 `A_TO_B`、兼容的 `1.0.0-candidate`、`embedded_constitutive_trial`。 |
...
- B 必须拒绝未知的主版本；候选版本是否兼容由显式兼容表决定，不能只比较字符串大小。
- 新增可选诊断字段可在不改变语义时向后兼容；改变 wrench 方向、参考点默认、单位、事件分数定义、状态所有权、提交语义或每针载荷边界必须升级合同主版本。
- 参数包、材料模型和数值配置可独立升级，但响应必须回传完整版本 ID。
- 本候选合同转为正式版前，必须完成第 7 节全部合同测试以及集成模型第 10–11 节的实现/实验关闭条件；“规范已写出”不等于代码或实验已验证。
