...
EXPECTED_UPLOADS = (
    ...
    "docs/extract/机理提取/20_gravity_independent_microspine_grippers/20_gravity_independent_microspine_grippers.zip",
)
...
def main() -> int:
    ...
    assert upstream["module_integration_run"] == "B_INTEGRATION-r01"
    ...
    if sum(path.endswith(".zip") for path in paths) != 2:
        raise ValueError("C1 最小文献包必须恰为 2 个 zip")
    if "derivation/modules/B/final/B_INTEGRATED_MODEL.md" in paths:
        raise ValueError("C1 默认上传清单不得包含完整 B 集成模型")
    if any("C_MODULE_CONTEXT" in path for path in paths):
        raise ValueError("C1 是 C 首阶段，不应上传既有 C_MODULE_CONTEXT")
    for forbidden_literature in ("07_", "17_", "28_"):
        if any(Path(path).name.startswith(forbidden_literature) for path in paths):
            raise ValueError(f"C1 默认上传清单误含备用文献：{forbidden_literature}")
    ...
    prompt_path = ROOT / "derivation/prompts/C/C1_PROMPT.md"
    ...
    if archive_content != prompt_bytes:
        raise ValueError("PROMPT.md 与正式 C1_PROMPT.md 不是逐字节一致")
    ...
    if "`pass / accepted`" not in report_content:
        raise ValueError("B 集成验证报告未确认 pass / accepted")
    ...
    for marker in (
        ...
        if marker not in integrated_content:
            raise ValueError(f"B 正式集成模型身份不完整：{marker}")
    ...
    for key in (
        ...
        "upstream_b_integration_validation",
    ...
    upstream_input_output = run_python_check(
        "derivation/runs/B_INTEGRATION/validate_inputs.py"
    )
    upstream_artifact_output = run_python_check(
        "derivation/runs/B_INTEGRATION/validate_artifacts.py"
    )

    print("C1-r01 输入准备校验通过")
    print("- B_INTEGRATION-r01 与 B_TO_C_CONTRACT 1.0.0 已接受且哈希一致")
    print("- 10 个上传文件的路径、顺序、字节数和 SHA-256 一致")
    print("- 冻结 Git 提交中的全部上传输入与 manifest 一致")
    print("- 正式提示词与运行归档副本逐字节一致")
    print("- 文献 09、20 压缩包结构、关键图数量和 CRC 通过")
    print("- 网页输出严格为 C_MODULE_CONTEXT、工程事实候选、运行摘要和引用说明")
