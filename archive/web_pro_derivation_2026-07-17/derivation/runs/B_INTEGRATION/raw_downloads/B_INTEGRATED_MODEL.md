...
本候选集成已关闭 B1/B2/B3 的语义重复、状态所有权、参考点、功方向、主动推力、事件事务和 C 嵌入提交污染问题；未关闭项均保持参数化、不可用或显式能力缺口。没有开始 C 子模块，也没有生成新的工程事实。
...
| `B_model` | `B_INTEGRATED_MODEL 1.0.0-candidate` |
...
- 关闭该缺口需要版本化的 B 六维/转动运动扩展、逐针姿态更新、碰撞/表面查询、事件、切线与验证合同；不属于本候选版本。
