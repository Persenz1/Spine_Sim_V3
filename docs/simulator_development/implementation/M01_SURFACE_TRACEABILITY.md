...
- 详细证据见 [`M01_VALIDATION_REPORT.md`](../../../reports/m01/M01_VALIDATION_REPORT.md) 与 [`M01_ACCEPTANCE_REPORT.md`](../../../reports/m01/M01_ACCEPTANCE_REPORT.md)。机器可读性能 JSON、canonical bundle、PNG 和 plot manifest 由下列命令在本地重建，默认不追踪。
...
| §12 optional 2D/3D preview、low-pass、plot manifest | `surface/preview/` | `test_demo_preview.py`；本地重建的六张 demo 图及各自 plot manifest | PASS |
| §13 numerical/resource/performance limits | `materialization.py`, `scripts/run_m01_performance.py` | `test_surface_performance.py`；本地重建的 `M01_PERFORMANCE_REPORT.json` | PASS |
