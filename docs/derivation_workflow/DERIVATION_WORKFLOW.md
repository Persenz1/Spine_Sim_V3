...
12. 每次网页执行都必须有唯一运行编号 `<TASK>-rNN`，例如 `A1-r01`。同一任务返工时不得覆盖既有原始回答或候选文件；首次执行沿用 `derivation/runs/<TASK>/`，后续执行使用 `derivation/runs/<TASK>-rNN/`。运行编号和实际目录必须同时写入输入清单与运行摘要。
...
所有 A1–C3 子模块默认上传以下共同输入，提示词必须逐项列出准确路径，实际上传不得省略：

1. 最新 `engineering_fixed_context/engineering_fixed_context.md`；
2. `docs/extract/MECHANISM_MODULE_PLAN.md`；
3. 当前子模块提示词；
4. `docs/derivation_workflow/templates/MODULE_CONTEXT_TEMPLATE.md`；
5. `docs/derivation_workflow/templates/ENGINEERING_FIXED_CONTEXT_CANDIDATE_TEMPLATE.md`；
6. `docs/derivation_workflow/templates/RUN_UPDATE_SUMMARY_TEMPLATE.yaml`；
7. `docs/derivation_workflow/templates/CITATION_BRIEF_TEMPLATE.md`；
8. 当前子模块最终最小文献包，逐个压缩包列名。

当前模块上下文或上游合同按 5.1–5.4 追加。`LITERATURE_MODULE_ROUTING.md`、`DERIVATION_WORKFLOW.md` 和两份本地指导由 Codex 阅读，其有效约束写入提示词，不默认上传给网页端。

### 5.1 A1

只上传上述共同输入，不追加既有模块上下文或上游合同。
...
与 A1 有关的文献角色、工作流要求和输出规则必须已经写入 `A1_PROMPT.md`，避免全量管理文档占用推理上下文。

### 5.2 A2 和 A3

在上述共同输入之外，追加最新完整 `derivation/modules/A/current/A_MODULE_CONTEXT.md`。

A2 输出融合 A1+A2 的完整 `A_MODULE_CONTEXT`；A3 输出融合 A1+A2+A3 的完整 `A_MODULE_CONTEXT`。不得只输出本轮增量而丢失此前已接受内容。

### 5.3 B1、B2、B3

B1 在共同输入之外追加 `derivation/contracts/A_TO_B_CONTRACT.md`。B2、B3 除共同输入外，追加该合同和最新完整 `derivation/modules/B/current/B_MODULE_CONTEXT.md`。

除非需要审计 A 层具体推导，B 层不默认上传完整 `A_INTEGRATED_MODEL` 或 A1–A3 历史快照。

### 5.4 C1、C2、C3

C1 在共同输入之外追加 `derivation/contracts/B_TO_C_CONTRACT.md`。C2、C3 除共同输入外，追加该合同和最新完整 `derivation/modules/C/current/C_MODULE_CONTEXT.md`。
...
- 只保存到本轮实际 `run_directory` 下的 `CITATION_BRIEF.md`；
