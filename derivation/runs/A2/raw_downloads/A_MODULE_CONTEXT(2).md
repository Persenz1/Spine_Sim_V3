...
> 当前完成阶段：`A2`——单边接触、摩擦稳定与结构柔顺加载  
> 上下文候选版本：`0.2.0`  
> 工程事实版本：`engineering_fixed_context 1.0.0`  
> 执行提示词版本：`A2 1.0.0`  
> 当前状态：`candidate`

本文档是大模块 A 截至 A2 完成后的最新完整滚动上下文，不是 A2 增量。第 1–12 节及附录 A–B 完整保留已接受的 A1 几何、数据质量、事件、验证和交接合同；第 13 节起新增 A2 的单边接触、三维库仑摩擦、刚性/梁针/轴向弹簧统一柔顺、恒法向主动推力混合边界、非光滑单步求解及 A3/B 接口。固定工程事实以事实 ID 标注；本地证据沿用 A1 的 `[L01]`、`[L11]`、`[L12]`、`[L16]`，并新增 `[L07]`；外部原始或官方来源记为 `[E-*]`；通用刚体运动学、梁理论、互补、变分不等式和数值分析知识记为 `[GPT]`。这些标记只说明证据层级，不把论文示例数值或数值算法参数升级为工程固定值。
...
在滑移边界有 $W=\lambda_nD$、$F=\lambda_nN$。因此必须分支处理：

- $W>0,D>0$：存在有限临界比 $F/W=N/D$，且 $\lambda_n=W/D>0$；
- $W>0,D=0$：有限 $\lambda_n$ 的滑移边界不能平衡正 $W$，标量比发散；在理想刚性库仑模型中进入水平自锁临界；
- $W>0,D<0$：对规定 $+x$ 扰动，摩擦不等式不再给出有限水平滑移上限，属于理想水平自锁区，但仍受结构、体碰撞和材料接口限制；
