...
| 3. §14 tests | 全量 `pytest` 179 passed；lint、format、strict typing 全通过 | ACCEPTED |
...
| [`M01_PERFORMANCE_REPORT.json`](M01_PERFORMANCE_REPORT.json) | `overall_pass=true`；13/13 checks true；64 MiB cache 受控 |
...
| [`demo/`](demo/) | gentle/medium/sharp 各 2D/3D PNG 及六份 plot manifest |
| package artifacts | `spine_sim-0.2.0.tar.gz` 与 `spine_sim-0.2.0-py3-none-any.whl` 构建成功 |

`build/` 与 `dist/` 中的大型运行/打包产物按仓库策略保持可重建，不作为 source artifact 依赖；签入的是报告、机器可读性能证据、图和 plot manifests。
