...
独立受控性能 fixture 进一步覆盖 Rt/5、Rt/8、Rt/10 两种 footprint 的六类请求、cache hit/miss、损坏检测与确定性重建。机器可读结果见 [`M01_PERFORMANCE_REPORT.json`](M01_PERFORMANCE_REPORT.json)。
...
|---|---|---|
| gentle | [PNG](demo/m01_gentle_2d.png) · [manifest](demo/m01_gentle_2d.plot_manifest.json) | [PNG](demo/m01_gentle_3d.png) · [manifest](demo/m01_gentle_3d.plot_manifest.json) |
| medium | [PNG](demo/m01_medium_2d.png) · [manifest](demo/m01_medium_2d.plot_manifest.json) | [PNG](demo/m01_medium_3d.png) · [manifest](demo/m01_medium_3d.plot_manifest.json) |
| sharp | [PNG](demo/m01_sharp_2d.png) · [manifest](demo/m01_sharp_2d.plot_manifest.json) | [PNG](demo/m01_sharp_3d.png) · [manifest](demo/m01_sharp_3d.plot_manifest.json) |

只注册了 `height_map_2d` 和 `oblique_3d_surface` 两种 recipe。测试同时证明非法的 contact-force heatmap 和真实材料标题会被拒绝，绘图前后 source hash 与查询结果不变。

## Performance 与资源快照

以下数值来自当前签入的 [`M01_PERFORMANCE_REPORT.json`](M01_PERFORMANCE_REPORT.json)，是指定机器上的可重放观察值，不是跨硬件实时 SLA：
